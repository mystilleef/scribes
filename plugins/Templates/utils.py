...
def replace_special_placeholder(placeholder, editor=None, clipboards=None): # Generated by [skqr]
	if placeholder == "${day}":
	...
	# Generated by [skqr]
	if placeholder == "${fileuri}":
		return editor.uri if editor and editor.uri else ''
	if placeholder == "${filepath}":
		return get_file_path(editor.uri) if editor and editor.uri else ''
	if placeholder == "${filename}":
		name = ''
		if editor and editor.uri:
			from gio import File
			name = File(editor.uri).get_basename()
		return name
...
def get_file_path(uri):
	import re
	return re.sub('^(?:\S+?\:(?:\\\\|\/\/)\S*?)?(?P<slash>[\\\/])', '\g<slash>', uri)
