...
class ReplaceBarTrigger(GObject):
	"""
	This class implements an object that shows the text editor's replace bar.
	"""
	...
	def __findbar_cb(self, trigger):
		...
		try:
			...
			from Manager import ReplaceBar
			self.__manager = ReplaceBar(self.__editor)
			self.__manager.show_bar()
		return
