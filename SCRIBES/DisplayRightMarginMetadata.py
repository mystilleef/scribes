...
def open_database(flag="c"):
	"""
	Open the display right margin database.

	@return: A database object representing the right margin database.
	...
	database_file = join(preference_folder, "DisplayRightMargin.gdb")
...
def get_value():
	"""
	Get display right margin properties from database.

	@return: A  display right margin property
...
def set_value(value):
	"""
	Set display right margin property in database.

	@param value: Right margin property
