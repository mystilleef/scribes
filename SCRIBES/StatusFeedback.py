class Feedback(object):
	...
	def __set_default_message(self):
		try:
			...
			self.__set_label(filename + " [modified]", italic=True) if self.__editor.modified else self.__set_label(filename, bold=True)
		except ValueError:
			self.__set_label(self.__queue[-1], True, False, "blue")
		except TypeError:
			self.__set_label()
		finally:
			self.__busy = False
	...
	def __update_message(self, message, time):
		self.__busy = True
		self.__set_label(message, True, False, "blue")
		from gobject import timeout_add
		timeout_add(time*1000, self.__set_default_message, priority=9999)
		return False

	def __set_message(self, message, color="blue"):
	...
	def __loaded_file_cb(self, editor, uri, *args):
		...
		from gobject import idle_add
		idle_add(self.__update_message, message, 7, priority=9999)
		message = "Loading %s" % filename
	...
	def __update_message_cb(self, editor, message, icon_name, time):
		from gobject import idle_add
		idle_add(self.__update_message, message, time, priority=9999)
		return False
