...
"""
This module documents functions to store and get the text wrapping 
properties from the text wrapping database.
...
def open_database(flag="c"):
	"""
	Open the text wrapping database.

	@return: A database object representing the text wrapping database.
	...
	database_file = join(preference_folder, "TextWrapping.gdb")
...
def get_value():
	"""
	Get text wrapping properties from database.

	@return: A  text wrapping property
...
def set_value(value):
	"""
	Set text wrapping property in database.

	@param value: Text wrapping property
