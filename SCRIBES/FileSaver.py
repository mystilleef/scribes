class Saver(object):
	...
	def __save(self, uri, encoding, check_saving=True):
		try:
			if check_saving and self.__is_saving: raise AttributeError
			self.__is_saving = True
			from gobject import idle_add
			idle_add(self.__send_data_to_processor, uri, encoding, priority=9999)
		except AttributeError:
			print "new save job added to queue"
	...
	def __check_queue(self, uri, encoding):
		try:
			...
			from gobject import idle_add
			idle_add(self.__save, uri, encoding, False, priority=9999)
		except IndexError:
			self.__is_saving = False
	...
	def __dbus_save_error_cb(self, editor, uri, encoding, message):
		from gobject import idle_add
		idle_add(self.__emit_error, priority=9999)
		return False
