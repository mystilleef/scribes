...
class ScribesMessageDialog(MessageDialog):
	"""
	...
	def show_message(self, message, title_message=None, parent_window=None):
		"""
		...
		if parent_window:
			...
			if parent_window.get_property("name") in ["Text Editor Window"]: self.__use_signals = True
		if title_message: self.format_secondary_markup(title_message)
	...
	def __set_properties(self):
		...
		@type self: A ScribesErrorDialog object.
		"""
		self.set_property("name", "scribes message dialog")
		from gtk import STOCK_CLOSE, RESPONSE_CLOSE
		self.add_button(STOCK_CLOSE, RESPONSE_CLOSE)
		self.set_property("title", "Information")
		self.set_property("role", "message_dialog")
