class Button(object):
	...
	def __init_attributes(self, manager, editor):
		...
		self.__button = manager.gui.get_object("MarginCheckButton")
	...
	def __update(self, language=""):
		...
		value = get_value(self.__language)
		self.__button.set_active(value)
		self.__manager.emit("margin-display", value)
		self.__button.handler_unblock(self.__sigid1)
		self.__button.set_property("sensitive", True)
		return 

	def __set(self):
		from SCRIBES.DisplayRightMarginMetadata import set_value
		set_value((self.__language, self.__button.get_active()))
		return 

	def __reset(self):
		from SCRIBES.DisplayRightMarginMetadata import reset
